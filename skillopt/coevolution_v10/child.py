"""Trusted child harness, appended to codec.py and run only in the OS sandbox.
...
def _main(encode_value, decode_value):
    ...
    for case in payload["cases"]:
        ...
        try:
            ...
            rows.append({"value": None, "exception": type(error).__name__})
        else:
            try:
                encoded = encode_value(value)
            except (ValueError, TypeError, OverflowError, RecursionError):
                unsupported = True
                rows.append({"value": None, "exception": None})
            else:
                rows.append({"value": encoded, "exception": None})
    encoded = json.dumps({"observations": rows, "unsupported_output": unsupported}, allow_nan=False)
