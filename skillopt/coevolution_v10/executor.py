"""V10 bounded typed single-function runner; never executes candidates in host.
...
from skillopt.coevolution_v10.child import AVAILABLE_BUILTINS, IMPORT_MEMBERS
...
VERSION = "coevolution-v10-typed-executor-v1"
_DIRECTORY = Path(__file__).resolve().parent
CHILD_RUNNER = (
    (_DIRECTORY / "codec.py").read_text(encoding="utf-8")
    + "\n"
...
def run_cases(code: str, entry_point: str, cases: list[dict]) -> dict:
    ...
    Unsupported return types remain an explicit compatibility failure category.
    """
    result = {
    ...
    try:
        ...
        if type(decoded) is not dict or set(decoded) != {"observations", "unsupported_output"}:
            raise ValueError("invalid child envelope")
        if type(decoded["unsupported_output"]) is not bool:
            raise ValueError("invalid unsupported output flag")
        ...
        unsupported_rows = 0
        for row in rows:
            if type(row) is not dict or set(row) != {"value", "exception"}:
            ...
            if row["exception"] is not None:
                ...
                decode_value(row["value"])
            else:
                unsupported_rows += 1
        if bool(unsupported_rows) != decoded["unsupported_output"]:
            raise ValueError("unsupported output flag mismatch")
        ...
        result["status"] = "unsupported_output" if unsupported_rows else "completed"
        if unsupported_rows:
            result["error_category"] = "unsupported_return_type_or_encoding_bound"
