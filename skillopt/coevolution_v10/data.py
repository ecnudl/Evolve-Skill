...
VERSION = "v10-mbpp-sanitized-data-v1"
...
def _implementation_hashes():
    ...
    return {name: file_hash(root / name) for name in ("data.py", "assertions.py", "codec.py", "executor.py", "child.py")}
...
def _history(repo, run_dir, records):
    ...
    for path in _history_paths(repo, run_dir):
        ...
        if path.name == "eligibility_manifest.json" and path.is_relative_to(repo / "outputs/coevolution_v10"):
...
def prepare_manifest(repo, run_dir, counts, seed, source_path=None):
    ...
    if run_dir == registry or not run_dir.is_relative_to(registry):
        raise ValueError("Use a separate run beneath outputs/coevolution_v10")
...
def materialize_split(repo, manifest, split, *, final_authorization=None):
    ...
    if not root.is_relative_to(repo / "outputs/coevolution_v10") or _read(root / "data_manifest.json") != manifest:
