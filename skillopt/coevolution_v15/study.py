...
from skillopt.coevolution_v14.study import source_hashes as previous_sources
from skillopt.validator_pilot.api import digest

from . import analysis, evidence, learning, runtime, tasks, validator

VERSION = "v15-lagged-calibrated-coevolution-pilot-v1"
SEED = 2026091505
...
def safe_root(repo, output):
    ...
    parent = repo / "outputs/coevolution_v15"
    if (any(p.is_symlink() for p in (raw, *raw.parents)) or ".." in raw.parts
            or raw == parent or not raw.is_relative_to(parent)):
        raise ValueError("Use a dedicated nonsymlink output beneath outputs/coevolution_v15")
...
def source_hashes(repo):
    ...
    paths = list((repo / "skillopt/coevolution_v15").glob("*.py"))
    paths += [repo / "scripts/coevolution_v15.py", repo / "scripts/launch_coevolution_v15.py",
              repo / "skillopt/validator_document_transport.py", repo / "docs/coevolution-v15-protocol.md"]
