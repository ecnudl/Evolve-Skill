"""V15 symmetric delivery-guarded two-draw solver with immutable native execution receipts.
...
from .tasks import public_task as task_public

VERSION = "v15-symmetric-delivery-guard-runtime-v1"
TOKENS = 8500
MAX_REQUEST_CHARS = 300000
MAX_TRACE_CHARS = 60000
PHASES = {"development", "calibration", "selection", "final"}
...
def _phase(task, phase):
    if phase not in PHASES:
        raise ValueError("Use an explicit development, calibration, selection or final phase")
    split = task.get("split")
    allowed = {"development": {"development", "dev", "train"}, "calibration": {"calibration"},
...
def _system(domain):
    ...
    precision = ("Spreadsheet formulas have one leading =; equality INSIDE formulas uses ==, never single =. "
                 "Example: =IF(A1==0,0,A2/A1). Follow the declared lazy IF and finite arithmetic. "
                 if domain == "spreadsheet" else
                 "For edits return the COMPLETE original rule list, INCLUDING every protected rule exactly unchanged. "
                 "Do not output only edited rules. Keep all original rule IDs in generation and apply.artifact. ")
    return common + precision + ("Use exact JSON. On generation return the standard artifact directly: " + artifact + ". "
...
def _request(api, root, system, user, *, identity, stage, initial_hash=None, completed=False):
    ...
    arguments = {"system": system, "user": user, "kind": "v15_solve_" + stage,
...
def solve(adapter, api, skill, *, key, repeat=0, root: Path, phase="development", completed=False):
    ...
    identity = {"version": VERSION, "task_hash": digest(task), "public_task_hash": digest(task_public(adapter)),
...
def evaluate_probe(adapter, artifact, *, root: Path, key, completed=False, phase=None):
    """One actual development/calibration counterexample evaluation, without a model.
    ...
    task = legacy.payload(adapter)
    actual_phase = task.get("split")
    if actual_phase not in {"development", "calibration"} or phase not in {None, actual_phase}:
        raise ValueError("Probe evaluation requires the actual development/calibration phase")
    phase = actual_phase
    if task.get("metadata", {}).get("probe") is not True:
        raise ValueError("Only an explicitly source-bound probe adapter can use probe evaluation")
    ...
    private = task.get("private_cases", task.get("hidden_cases", []))
    input_field = "input" if adapter.domain == "coding" else "overrides"
    inputs = [case.get(input_field) for case in cases]
    if (adapter.domain not in {"coding", "spreadsheet"} or private or not 1 <= len(cases) <= 4
            or metadata.get("partition") != phase
            or type(metadata.get("source_task_id")) is not str or not metadata["source_task_id"]
            or not re.fullmatch(r"[a-f0-9]{64}", str(metadata.get("source_task_hash")))
            or metadata.get("probe_input_hashes") != [digest(value) for value in inputs]
            or task["id"] != metadata["source_task_id"] + "-probe-" + digest(inputs)[:16]):
        raise ValueError("Probe source, inputs, phase or case grid is not properly bound")
    source = {name: deepcopy(metadata[name]) for name in ("source_task_id", "source_task_hash", "case_obligations") if name in metadata}
    identity = {"version": VERSION, "phase": phase, "probe": True,
    ...
    evaluation, execution_hash, execution_id = _evaluate(adapter, artifact, directory,
        public_only=False, identity=identity, completed=replay)
    return _save(path, {"version": VERSION, "identity": identity, "phase": phase,
