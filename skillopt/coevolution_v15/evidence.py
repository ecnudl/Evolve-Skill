"""Read-only, bounded development feedback from already closed V15 receipts.

This is NOT an optimizer, execution engine, safety gate, or efficacy result.
The caller supplies an audited immutable V15 solve and its durable dependencies.
...
VERSION = "v15-closed-development-feedback-v1"
SOURCE_VERSION = "v15-symmetric-delivery-guard-runtime-v1"
...
def project_development_feedback(solve, stages, *, api_receipts, executions, max_bytes=32768) -> DevelopmentFeedback:
    ...
    try:
        ...
        raise ValueError("Unsupported or incomplete closed V15 evidence schema") from error
...
def _project(solve, stages, api_receipts, executions, max_bytes):
    ...
    for index, (stage, api) in enumerate(zip(stages, api_receipts)):
        ...
        _require(request["kind"] == "v15_solve_" + name and request["key"] == expected_key
        ...
        if diagnostic is not None:
            _require(type(diagnostic) is dict and set(diagnostic) <= {"stage", "type", "message", "exception"}
                     ...
                     "Delivery diagnostics must preserve the bounded V15 schema")
