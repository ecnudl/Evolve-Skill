...
from skillopt.coevolution_v10.study import source_hashes as parent_source_hashes
from skillopt.validator_pilot.api import digest

from . import analysis, data, execution, executor

VERSION = "v11-frozen-searchqa-coding-transfer-v1"
DESIGNS = {
    "smoke": {"confirmation": 2, "final": 4, "seed": 202609143},
    "formal": {"confirmation": 64, "final": 128, "seed": 202609144},
}
...
def source_hashes(repo):
    ...
    paths = list((repo / "skillopt/coevolution_v11").glob("*.py"))
    paths.extend(repo / name for name in (
        "scripts/coevolution_v11.py", "scripts/report_coevolution_v11.py", "scripts/launch_coevolution_v11.py",
        "scripts/audit_coevolution_v9.py", "docs/coevolution-v11-protocol.md",
...
def _safe_run(repo, root):
    ...
    root = root.resolve()
    parent = repo / "outputs/coevolution_v11"
    if root == parent or not root.is_relative_to(parent):
        raise ValueError("Use an isolated run beneath outputs/coevolution_v11")
    return root
...
class Study:
    ...
    def prepare(self):
        ...
        self.protocol = _save(self.root / "protocol.json", {
            ...
            "truthiness": "uniform_trusted_child_observation_without_private_predicates",
            "generation_contract": "single_python_fenced_block_no_json_wrapper",
            "supersedes_engineering_design_not_old_results": "v10_smoke_20260914",
    ...
    def _grid(self, api, tasks, split):
        ...
        for offset in range(0, len(jobs), 4):
            self._pause_check()
            observed = api.parallel(jobs[offset:offset + 4], solve, "v11-" + split)
