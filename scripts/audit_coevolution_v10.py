"""Read-only, score-blind V10 transport progress; no API or candidate execution."""
...
from scripts.coevolution_v10 import _read, completed_replay, safe_root  # noqa: E402
...
def progress(output, repo=REPO):
    ...
    for path in (root / "api/calls").glob("*.json"):
        ...
        if (row.get("request_hash") != path.stem or digest(row["request"]) != path.stem
                or type(row.get("ok")) is not bool or row["request"].get("kind") != "v10_coding_solve"):
