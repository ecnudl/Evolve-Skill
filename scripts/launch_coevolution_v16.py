"""Supervise one V16 run; preserve interruptions, no automatic resampling."""
...
def main(argv=None):
    from skillopt.coevolution_v16.study import DESIGNS, safe_root
    ...
    if (any(p.is_symlink() for p in (report, *report.parents)) or report.suffix != ".md"
            or not report.is_relative_to(REPO / "docs") or report.name.endswith("-protocol.md")):
    ...
    with (root / "supervisor.lock").open("a+") as lock:
        ...
        with (root / "supervisor.log").open("a", buffering=1) as log:
            ...
            command = [sys.executable, "-B", "-u", str(REPO / "scripts/coevolution_v16.py"),
