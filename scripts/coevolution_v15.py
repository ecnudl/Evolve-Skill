"""Run, explicitly resume, or read-only replay one preregistered V15 study."""
...
def write_report(root, target):
    from skillopt.coevolution_v15.reporting import render
    path = Path(target).absolute()
    if (any(p.is_symlink() for p in (path, *path.parents)) or path.suffix != ".md"
            or not path.is_relative_to(REPO / "docs") or path.name == "coevolution-v15-protocol.md"):
