"""Run one V11 study and publish its offline-verified report; never auto-retry."""
...
from scripts.coevolution_v11 import safe_root  # noqa: E402
...
def main(argv=None):
    ...
    with (root / "supervisor.lock").open("a+") as lock:
        ...
        with (root / "supervisor.log").open("a", buffering=1) as log:
            command = [sys.executable, "-B", "-u", str(REPO / "scripts/coevolution_v11.py"),
            ...
            code = subprocess.run([sys.executable, "-B", str(REPO / "scripts/report_coevolution_v11.py"),
