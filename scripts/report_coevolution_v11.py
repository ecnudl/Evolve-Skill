...
from scripts import coevolution_v11 as runner  # noqa: E402
...
def render(protocol, result, audit):
    ...
    lines = ["# V11 冻结 SearchQA Skill → Coding 迁移报告", "",
...
def _target(report, root, repo, protocol):
    target = Path(report).absolute() if report is not None else repo / "docs" / f"coevolution-v11-{root.name}-report.md"
