...
def _study_module():
    ...
    return importlib.import_module("skillopt.coevolution_v11.study")
...
def safe_root(output, repo=REPO):
    ...
    if root == registry or not root.is_relative_to(registry):
        raise ValueError("Use a separate output beneath outputs/coevolution_v11")
    return root
...
def _completed_replay(root, repo):
    ...
    audit = core.seal({"version": "v11-completed-cli-replay-v1", "result_hash": result["record_hash"],
