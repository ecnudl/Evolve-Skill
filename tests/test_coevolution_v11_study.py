"""Offline full V11 transfer using a synthetic 427-row dataset and fake I/O.
...
from skillopt.coevolution_v11 import data, execution, executor
from skillopt.coevolution_v11 import study as s
...
def fixture(tmp_path, monkeypatch):
    ...
    def fake_child(code, entry_point, cases):
        ...
        observations = [{"value": executor.encode_value("WRONG") if wrong else case["args"][0],
                         "exception": None, "typed": True,
                         "truthiness": bool(executor.decode_value(executor.encode_value("WRONG") if wrong else case["args"][0]))}
                        for case in cases]
