...
from skillopt.coevolution_v11 import data
...
def _prepare(panel, name="run", counts=None, seed=13):
    repo, _ = panel
    return data.prepare_manifest(repo, repo / "outputs/coevolution_v11" / name, COUNTS if counts is None else counts, seed)
...
def test_snapshot_assets_and_release_are_verified(panel):
    ...
    assert str(data.DIRECTORY) == "data/coevolution_v10/mbpp_sanitized_08a8d673"
    assert data.SOURCE_VERSION == "v10-mbpp-sanitized-official-source-v1"
...
def test_static_compilation_does_not_execute_reference(panel, monkeypatch):
    ...
    assert set(report["implementation_hashes"]) == {
        "skillopt/coevolution_v11/data.py", "skillopt/coevolution_v11/assertions.py",
...
def test_v10_selected_and_unused_final_reservations_excluded_without_consuming_catalog(panel):
    from skillopt.coevolution_v10 import data as old_data

    repo, _ = panel
    first = old_data.prepare_manifest(repo, repo / "outputs/coevolution_v10/smoke", COUNTS, 42)
    second = _prepare(panel)
    assert not {row["task_id"] for row in _selected(first)} & {row["task_id"] for row in _selected(second)}
    assert second["pool_counts"]["confirmation:eligible_unique"] == 161
    assert second["pool_counts"]["final:eligible_unique"] == 253
    assert second["exposure_counts"]["ids"] == 8
    inventory = data._read(repo / second["run_path"] / "exposure_inventory.json")
    catalog = next(row for row in inventory["files"] if row["path"].endswith("eligibility_manifest.json"))
    assert catalog["matched_ids"] == catalog["matched_questions"] == 0


@pytest.mark.parametrize("registry", ["coevolution_v10", "coevolution_v11"])
def test_unknown_eligibility_catalog_fails_closed(panel, registry):
    repo, _ = panel
    _write(repo / "outputs" / registry / "old/eligibility_manifest.json", core.seal({"version": "unknown"}))
...
def test_invalid_counts(panel, counts):
    with pytest.raises(ValueError, match="counts"):
        data.prepare_manifest(panel[0], panel[0] / "outputs/coevolution_v11/run", counts, 1)
...
@pytest.mark.parametrize("location", ["outputs/coevolution_v11", "outputs/coevolution_v9/run", "data/run"])
...
def test_not_enough_candidates_fails_without_partial_reservation(panel):
    ...
    assert not (panel[0] / "outputs/coevolution_v11/run/data_manifest.json").exists()
...
def test_deterministic_sampling_separate_identical_histories(panel, tmp_path):
    ...
    second = data.prepare_manifest(other, other / "outputs/coevolution_v11/run", COUNTS, 13)
