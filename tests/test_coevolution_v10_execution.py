...
from skillopt.coevolution_v10 import assertions, data, executor
from skillopt.coevolution_v10 import execution as e
...
def fixture(tmp_path, monkeypatch):
    ...
    def child(code, entry, cases):
        ...
        if status == "completed":
            ...
            result["observations"] = [{"value": encode_value(value), "exception": None} for value in values]
        elif status == "unsupported_output":
            result["observations"] = [{"value": None, "exception": None} for _ in cases]
        return result
    ...
    class API:
        ...
        def call(self, system, user, kind, key, *, max_tokens, repeat):
            ...
            result = {"request": request, "request_hash": identifier, "ok": state["api_ok"],
                      "response": json.dumps({"code": state["candidate"]}) if state["api_ok"] else "",
