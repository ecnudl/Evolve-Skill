# V10：冻结 SearchQA Skill 的 Coding 迁移与适用范围门控

预登记日期：2026-09-14。本协议在首次 V10 模型调用前固定；不修改 V9。
...
宿主只静态解析断言和比较 typed 值；代码执行仅在 macOS OS sandbox。子进程不接收私有 expected。保留 tuple/list/set/dict/bool/int/float/None 类型和 Python 精确比较；无浮点宽松容差。CPU 5s、墙钟 12s、RSS 384MiB 采样看门狗、无写文件/网络/凭据。资源失败（含 MemoryError）、非法执行回执、不可表示输出为 unknown，不伪装成语义错误。
...
| --- | ---: | ---: | ---: | ---: |
| smoke，seed 202609141 | 2 | 4 | 3 | 24 |
| formal，seed 202609142 | 64 | 128 | 3 | 768 |
